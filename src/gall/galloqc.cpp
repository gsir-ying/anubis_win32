...
{   
  ...
  for( itGOBS = stt.at(gs).begin(); itGOBS != stt.at(gs).end(); ++itGOBS )
  {
    ...
    if( gb != gobs2gobsband(gobs) ||
        gt != gobs2gobstype(gobs)   ) continue;

    for( itSAT = stt.at(gs).at(gobs).begin(); itSAT != stt.at(gs).at(gobs).end(); ++itSAT )
    {
      signal_sel[gobs2gobsattr(gobs)] += itSAT->second.first;
    }     
  }
                              ...
                              t_map_sky_sat& sat_mask )
{
...
#ifdef DEBUG
  cerr << epoB.str_ymdhms("===epB: ") <<  epoE.str_ymdhms(" epE: ") << " " << _mapobj[site].size() << endl;
#endif
  ...
  {
#ifdef DEBUG
    cerr << epo.str_ymdhms("-->epo: ") <<   end.str_ymdhms(" end: ") << endl;
#endif
    ...
    for(itPRN = sats.begin(); itPRN != sats.end(); ++itPRN )
    {
      ...
      if( nav_beg == LAST_TIME || nav_end == FIRST_TIME ){ continue; }

      int nav_validity = t_gnav::nav_validity(t_gsys::char2gsys(prn[0]));
#ifdef DEBUG
      cerr << prn << nav_beg.str_ymdhms(" beg: ") << nav_beg.str_ymdhms(" end: ") 
           << " " << nav_validity << endl;
#endif
      // EXCEPTION IF NO DATA
      if( _mapobj[site].find(epo) == _mapobj[site].end() ){
#ifdef DEBUG
        cerr << epo.str_ymdhms("-->epo: ") <<   end.str_ymdhms(" end: ") << " initiate elev "+prn << endl;
#endif
        epo_data[prn] = epo;
      }
      ...
      else if( (itSAT = _mapobj[site][epo].find(prn)) != _mapobj[site][epo].end() )
      {
        ...
        if( epo_data.find(prn) == epo_data.end() ){
          ...
          {
#ifdef DEBUG
            cerr << epo.str_ymdhms("-->epo: ") <<   end.str_ymdhms(" end: ") << " NO initiate "+prn << endl;
            cerr << itEPO->first.str_ymdhms("     search data .. "+prn+": ") << endl;
#endif
...
#ifdef DEBUG
          cerr << epo.str_ymdhms("   data initiate PRN: "+prn+": ") << endl;
#endif
        }
#ifdef DEBUG
        cerr << epo.str_ymdhms("-->epo: ") <<   end.str_ymdhms(" end: ") << " data initiated: "+prn << endl;
#endif
      }
      else{
#ifdef DEBUG
...
#ifdef DEBUG
      cerr << "EPO: " << epo_data[prn].str_ymdhms() << " size:" << epo_data.size() << endl;
#endif
      // ======================================
      ...
      while( search_back )
      {
...
#ifdef DEBUG
             cerr << prn+tt.str_ymdhms(" skip [NAV-2*3600]: ")+nav_beg.str_ymdhms(" ") << endl;
#endif          
...
#ifdef DEBUG
             cerr << prn+tt.str_ymdhms(" skip [NAV+2*3600]: ")+nav_end.str_ymdhms(" ") << endl;
#endif
...
#ifdef DEBUG
             cerr << prn+tt.str_ymdhms(" skip [EPO-2*3600]: ")+    epo.str_ymdhms(" ") << endl;
#endif
        ...
        if( gnav->nav( prn, tt, xyz_sat.crd_array(), NULL, NULL, false ) < 0 || xyz_sat.zero() ){
#ifdef DEBUG
          cerr << "INIT cannot find navigation message: "+prn+tt.str_ymdhms(" ") << endl;
#endif
          ...
          {
#ifdef DEBUG
            cerr << prn << " is unhealthy (set health) " << _useHealth << " \n";
#endif
...
#ifdef DEBUG
                cerr << tc.str_ymdhms("         hlt -> fill: "+prn+": ") << fixed << setprecision(3) << setw(8) << -9.0 << " " << health << endl;
#endif                
              }
            }
            search_back = false;
...
#ifdef DEBUG
          cerr << tt.str_ymdhms("        estimate ELE: "+prn+": ") << fixed << setprecision(3)
             ...
             ( itEPO != _mapobj[site].end() && itEPO->first < tepo[next]+1 ); ++itEPO )
          {
            ...
            if( _mapobj[site][tc].find(prn) != _mapobj[site][tc].end() ){
              ...
              _mapobj[site][tc][prn]->health(health);
#ifdef DEBUG
              cerr << tc.str_ymdhms("         ele -> fill: "+prn+": ") << fixed << setprecision(3) << setw(8) << ele << " " << health << endl;
#endif
            }
          }
...
#ifdef DEBUG
            cerr << tt.str_ymdhms("         ele -> skip: "+prn+": ")+dbl2str(elev[prev])+dbl2str(elev[next]) << endl;
#endif
            break;
          }else{ 
#ifdef DEBUG
            cerr << tt.str_ymdhms("         ele -> zero: "+prn+": ")+dbl2str(elev[prev])+dbl2str(elev[next]) << endl;
#endif
          }
          
          if( !health && _useHealth >= STT_HEALTH ){
#ifdef DEBUG
            cerr << prn << " is unhealthy (skip elev/azi) " << health << " " << _useHealth << " \n";
#endif
            continue;
         } // skip all unhealthy
          ...
          { 
...
#ifdef DEBUG
              cerr << tc.str_ymdhms(" .. INIT ele -> zero: "+prn+": ")+dbl2str(dele)+dbl2str(elev[prev])+dbl2str(elev[next]) << endl;
#endif
            }else{ break; } // stop
            ...
            if( tc > tepo[prev] ){
              ...
              epo_zero.erase(prn);
#ifdef DEBUG
              cerr << tc.str_ymdhms(" .. STOP ele -> zero: "+prn+": ")+dbl2str(dele)+dbl2str(elev[prev])+dbl2str(elev[next]) << endl;
#endif
            } // next iteration
          }
...
#ifdef DEBUG
            cerr << tt.str_ymdhms("         ele -> mask: "+prn+": ")+dbl2str(elev[prev])+dbl2str(elev[next]) << endl;
#endif
          }
          ...
          if( epo_mask.find(prn) == epo_mask.end() ){  // search mask (START/STOP)
...
#ifdef DEBUG
              cerr << tc.str_ymdhms(" .. INIT ele -> mask: "+prn+": ")+dbl2str(dele)+dbl2str(elev[prev])+dbl2str(elev[next]) << endl;
#endif
            }else break; // stop
            ...
            if( tc > tepo[prev] ){
              ...
              epo_mask.erase(prn);
#ifdef DEBUG
              cerr << tc.str_ymdhms(" .. STOP ele -> mask: "+prn+": ")+dbl2str(dele)+dbl2str(elev[prev])+dbl2str(elev[next]) << endl;
#endif
            } // next iteration
          }
        }       
