...
class t_gallobs : public t_gdata {
 ...
 public:
  ...
  virtual set<string> sats(const string& site, const t_gtime& t, GSYS gnss=GNS); // get all satellites for epoch t and system
...
//  virtual t_gsdbs sdbs(const string& site, const t_gtime& t);                  // single differ. btw sats in epoch t
  ...
  virtual set<t_gtime> epochs();                                                 // get all t_gepochs for site
