...
void t_gxtrqc::summary(string site)
{
  ...
  {
    if( _ele_new ){
//      sat_data( _qcdata,   _pos, _gnav, _beg, _end,      0.0 ); // NEW STYLE (QC-DATA)
...
//     
      ...
      _qcdata->qc_kpi.DF_XX[gsys] = ( _m_UnusEpo[gsys].first > _m_UnusEpo[gsys].second ) ?
                                      _m_UnusEpo[gsys].first : _m_UnusEpo[gsys].second ;
...
//    int ExpMask = _m_cutExp[gsys][X];              // Expected_CutMask + wo_Elev_Weighted (already included!)
      ...
      double ratH = ( ExpZero > 0 ) ? 100.0 * HavZero / ExpZero : 0.0;
      double ratU = ( ExpMask > 0 ) ? 100.0 * HavMask / ExpMask : 0.0;

      if( ratH > 100.0 ){ ratH = 100.0; ExpZero=HavZero; _m_obsExp[gsys][gobs] = _m_obsHav[gsys][gobs]; }
      if( ratU > 100.0 ){ ratU = 100.0; ExpMask=HavMask; _m_cutExp[gsys][gobs] = _m_cutHav[gsys][gobs]; }
...
//  _obs_end = _gobs->end_obs(_site);
    ...
    os << fixed << setprecision(4)
       ...
       << W(2)  << ""
       << W(16) << right << setrec->ecc(_obs_beg,t_gobj::ECC_XYZ).crd(0)
       << W(16) << right << setrec->ecc(_obs_beg,t_gobj::ECC_XYZ).crd(1)
       << W(16) << right << setrec->ecc(_obs_beg,t_gobj::ECC_XYZ).crd(2) << endl;
    ...
    os << fixed << setprecision(4)
       ...
       << W(2)  << ""
       << W(16) << right << setrec->ecc(_obs_beg,t_gobj::ECC_NEU).crd(1)          // E ! reverse with N
       << W(16) << right << setrec->ecc(_obs_beg,t_gobj::ECC_NEU).crd(0)          // N 
       << W(16) << right << setrec->ecc(_obs_beg,t_gobj::ECC_NEU).crd(2) << endl; // U
  }
}
...
// mean xyz
  ...
  for(auto itSYS = m_xyz.begin(); itSYS != m_xyz.end(); ++itSYS ){
     ...
     _qcdata->qc_est.xyz_crd[gsys] = _m_xyz_est[gsys] = stat.get_mean3d();
     _qcdata->qc_est.xyz_rep[gsys] = _m_xyz_rep[gsys] = stat.get_sdev3d();

     if( _qcdata->qc_est.xyz_rep[gsys][0] > 9999 ||
         _qcdata->qc_est.xyz_rep[gsys][1] > 9999 ||
         _qcdata->qc_est.xyz_rep[gsys][2] > 9999
     ){
       _qcdata->qc_est.xyz_crd[gsys][0] = _m_xyz_est[gsys][0] = 0;
       _qcdata->qc_est.xyz_crd[gsys][1] = _m_xyz_est[gsys][1] = 0;
       _qcdata->qc_est.xyz_crd[gsys][2] = _m_xyz_est[gsys][2] = 0;

       _qcdata->qc_est.xyz_rep[gsys][0] = _m_xyz_rep[gsys][0] = -9999;
       _qcdata->qc_est.xyz_rep[gsys][1] = _m_xyz_rep[gsys][1] = -9999;
       _qcdata->qc_est.xyz_rep[gsys][2] = _m_xyz_rep[gsys][2] = -9999;
     ...
     _setkey(os, str, '=',tt);
     os << fixed << setprecision(4)
        << W(16) << right << _qcdata->qc_est.xyz_crd[gsys][0]
        << W(16) << right << _qcdata->qc_est.xyz_crd[gsys][1]
        << W(16) << right << _qcdata->qc_est.xyz_crd[gsys][2]
                 << setprecision(1)
        << W(8)  << right << _qcdata->qc_est.xyz_rep[gsys][0]
        << W(8)  << right << _qcdata->qc_est.xyz_rep[gsys][1]
        << W(8)  << right << _qcdata->qc_est.xyz_rep[gsys][2]
        << W(6)  << right << size
        << W(6)  << right << outl
        << endl;       
  }
  ...
  for(auto itSYS = m_blh.begin(); itSYS != m_blh.end(); itSYS++){
     ...
     int size = stat.get_size();
     int outl = stat.get_outl();
     _qcdata->qc_est.blh_crd[gsys] = stat.get_mean3d();   //           blh = stat.get_mean3d();
     _qcdata->qc_est.blh_rep[gsys] = stat.get_sdev3d();   // t_gtriple sdev = stat.get_sdev3d();
     _qcdata->qc_est.blh_rep[gsys][0] *= A_WGS;           // convert to meters
     _qcdata->qc_est.blh_rep[gsys][1] *= A_WGS;           // convert to meters

     if( _qcdata->qc_est.blh_rep[gsys][0] > 9999 ||
         _qcdata->qc_est.blh_rep[gsys][1] > 9999 ||
         _qcdata->qc_est.blh_rep[gsys][2] > 9999
       ){
       _qcdata->qc_est.blh_rep[gsys][0] = -9999;
       _qcdata->qc_est.blh_rep[gsys][1] = -9999;
       _qcdata->qc_est.blh_rep[gsys][2] = -9999;
     }
     ...
     os << fixed << setprecision(9)
        << W(16) << right << _qcdata->qc_est.blh_crd[gsys][0]*R2D
        << W(16) << right << _qcdata->qc_est.blh_crd[gsys][1]*R2D
                 << setprecision(4)
        << W(16) << right << _qcdata->qc_est.blh_crd[gsys][2]
                 << setprecision(1)
        << W(8)  << right << _qcdata->qc_est.blh_rep[gsys][0]
        << W(8)  << right << _qcdata->qc_est.blh_rep[gsys][1]
                 << setprecision(1)
        << W(8)  << right << _qcdata->qc_est.blh_rep[gsys][2]
        << W(6)  << right << size
        << W(6)  << right << outl       
        << endl;
  }
...
void t_gxtrqc::_svinfo(ostringstream& os)
{
  ...
  for( auto itGSYS = set_sys.begin(); itGSYS != set_sys.end(); ++itGSYS )
  {
    ...
    set<t_gtime> epochs = _gnav->epochs(string(1,t_gsys::gsys2char(*itGSYS)));
    string key = gs + "HLT";
...
void t_gxtrqc::_xml_head()
{
  ...
  for(auto itHDR = _m_rnxHdr.begin(); itHDR != _m_rnxHdr.end(); ++itHDR, ++cnt )
  {
    ...
    _default_node(_XHDR, "receiver_vers",    trim(rnxhdr.recvers()).c_str(), true);
