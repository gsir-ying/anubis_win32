...
// Return value
// ----------
bool   t_gsetproc::clean_iono()
{
  _gmutex.lock();
   
  bool tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).attribute("clean_iono").as_bool();
...
// Return value
// ----------
string t_gsetproc::trop_corr()
{
	_gmutex.lock();

	string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).attribute("trop_corr").value();
...
// Return value
// ----------
string t_gsetproc::code_bias_src()
{
	_gmutex.lock();

	string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).attribute("code_bias_src").value();
...
// Return value
// ----------
string t_gsetproc::phase_bias_src()
{
	_gmutex.lock();

	string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).attribute("phase_bias_src").value();
...
ATTITUDES t_gsetproc::attitudes()
{
	...
	ATTITUDES AT = str2attitudes(tmp);

  if(AT == DEF_YAWMODEL){
    xml_node parent = _doc.child(XMLKEY_ROOT);
    xml_node node = _default_node(parent, XMLKEY_PROC);
    tmp = attitudes2str(_attitudes);
    _default_attr(node,"attitudes", tmp);
    
    AT = _attitudes;
  }
    
	_gmutex.unlock(); return AT;
}
...
CBIASCHAR t_gsetproc::cbiaschar()
{
	...
	CBIASCHAR CB = str2cbiaschar(tmp);

  if(CB == DEF_CBIASCHAR){
    xml_node parent = _doc.child(XMLKEY_ROOT);
    xml_node node = _default_node(parent, XMLKEY_PROC);
    tmp = cbiaschar2str(_cbiaschar);
    _default_attr(node,"cbiaschar", tmp);
    
    CB = _cbiaschar;
  }
    
	_gmutex.unlock(); return CB;
}
...
void t_gsetproc::check()
{
  ...
  _default_attr(node,"clean_iono", _clean_iono);  
  _default_attr(node,"auto_band", _auto_band);
  _default_attr(node,"code_bias_src",  _code_bias_src);
  _default_attr(node,"phase_bias_src", _phase_bias_src);     
...
// ----------   
ATTITUDES t_gsetproc::str2attitudes(string ati)
{
	ATTITUDES at;

	if (ati.compare("NOMINAL") == 0)  at = YAW_NOMI;
	else if (ati.compare("RTCM") == 0) at = YAW_RTCM;
	else {
		at = DEF_YAWMODEL;
		stringstream ostr;
		ostr << "Unsupported attitude model (" << at << ")! Used default value ( YAW_NOMI )";
...
// ----------   
CBIASCHAR t_gsetproc::str2cbiaschar(string cb)
{
   CBIASCHAR CB; 
   
        if(cb.compare("2CHAR") == 0) CB = CHAR2;
   else if(cb.compare("3CHAR") == 0) CB = CHAR3;
   else if(cb.compare("ORIG")  == 0) CB = ORIG;
   else {
      CB = DEF_CBIASCHAR;
      stringstream ostr;
...
OBSWEIGHT t_gsetproc::str2obsweight(string wg)
{
   ...
   else if(wg.compare("SINEL1") == 0) WG = SINEL;
...
{
   switch (CB) {
    case CHAR2:         return "2CHAR";
    case CHAR3:         return "3CHAR";
    case ORIG:          return "ORIG";
    case DEF_CBIASCHAR: return "NOT DEFINED";
   }
  return "";
}

// convert ATTITUDES enum to str
// ---------
string t_gsetproc::attitudes2str(ATTITUDES AT)
{
   switch (AT) {
    case YAW_NOMI:      return "YAW_NOMI";
    case YAW_RTCM:      return "YAW_RTCM";
    case DEF_YAWMODEL:  return "NOT DEFINED";
   }
  return "";
}   
