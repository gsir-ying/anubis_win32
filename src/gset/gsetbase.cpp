...
// re-read existing configuration
// ----------
int t_gsetbase::reread()
{
  if( ! _name.empty() && t_gsetbase::read(_name) == 0 ){
    cerr << "xconfig: " << _pgm << " re-configured [" << _name << "]\n";
    return 0;
  }

  cerr << "xconfig: no file used for configuration (skip reconfiguration)\n";
  return -1;
}
...
{	 
  ...
  cout << endl << "Usage: "
       ...
       << endl << "    -Z file                .. configuration output file          "
       << endl << "    -z                     .. log wout time stamp                "
...
//  else if( !strcmp(argv[i],"-m") && i  <argc ){ thrd = true; }
    ...
    else if( !strcmp(argv[i],"-z") && i  <argc ){ if(_log) _log->time_stamp(false); }
...
//  else if( !strcmp(argv[i],"-l") && i+1<argc ){ if(_log){ _log->mask(     argv[++i]);  }} // ZDE PRILIS BRZY!!
  ...
  if( !save.empty() ){ if( _log ) _log->comment(0, "gsetbase" , "xconfig: save to file "+_name );
    write(save); 
    if( conf.empty() ){ _name = save; } // for possible reconfiguration!
  }
...
{
  double num = 0.0;
//  string ELE = elem;    transform(ELE.begin(), ELE.end(), ELE.begin(), ::toupper);
//  string ATT = attrib;  transform(ATT.begin(), ATT.end(), ATT.begin(), ::toupper);

// num = _doc.child(XMLKEY_ROOT).child( ELE.c_str() ).attribute(ATT.c_str()).as_double();   
...
double t_gsetbase::_dblval( const string& elem, const string& subelem )
{
  double num = 0.0;
//  string ELE = elem;    transform(ELE.begin(), ELE.end(), ELE.begin(), ::toupper);
//  string SUB = subelem; transform(SUB.begin(), SUB.end(), SUB.begin(), ::toupper);

//istringstream is( _doc.child(XMLKEY_ROOT).child( ELE.c_str() ).child_value( SUB.c_str() ));
