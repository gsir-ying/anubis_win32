...
//		          const t_gtime& end = LAST_TIME );
          ...
          void chk_tot(bool b) { _chk_tot = b; }             // use tot < t condition
          void ext_nav(double d){ _ext_nav = d; }            // use extended window for navigation data
  ...
  virtual set<string> satellites(const set<string>& sats, const t_gtime& epo,
                                       t_gtriple& rec,    const double& min_ele);   // get satellites with valid nav message in a particular epoch
  ...
  virtual set<t_gtime> epochs( string prn = "" );                    // get list of nav epochs (all=default, G=system, G01=prn)
...
//		                       const t_gtime& end = LAST_TIME );
  ...
  double             _ext_nav;     // extend nav validity window
 //private:
};
