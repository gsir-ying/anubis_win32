...
set<t_gtime> t_gallobs::epochs()
{
  ...
  set<t_gtime> all_epochs;

  for( auto itSIT = _mapobj.begin(); itSIT != _mapobj.end(); ++itSIT ){
    for( auto itEPO = itSIT->second.begin(); itEPO != itSIT->second.end(); ++itEPO ){
      all_epochs.insert( itEPO->first );
    }
  }
...
//auto itEPO     = _mapobj[site].find(tt);
  ...
  // ===================================
  if( _overwrite || epo_found < 0                     // epoch does not exists (smart search)
                 || itSAT == _mapobj[site][tt].end()  // satellite does not exists
...
//                 || itSAT == _mapobj[site][tt].end()
    ...
    // comments
    if( _log && _log->verb() >= 4 ){
      vector<GOBS> v_obs = obs->obs(); auto itOBS = v_obs.begin();
      ostringstream lg; lg << fixed << setprecision(3);
      for( ; itOBS != v_obs.end(); ++itOBS ){
        lg << " " << gobs2str( *itOBS ) << ":" << obs->getobs( *itOBS );
      }
      _log->comment(4,"gallobs",site + tt.str_ymdhms(" add obs: ") + " " + sat + lg.str() );
    }
...
//                   && itSAT != _mapobj[site][tt].end()
        ...
        if( _log && _log->verb() >= 3 ) _log->comment(3, "gallobs", site + tt.str_ymdhms(" obs replaced ") + " " + sat);
    }
    ...
    if( _nepoch > 0 && _mapobj[site].size() > _nepoch + 10 ){  // +10 .. reduce number of removals to every tenths
      ...
      while( itEPO != _mapobj[site].end() ){
        ...
        if( _log && _log->verb() >= 3 )
        {
           ...
           _log->comment(3, "gallobs", site + t.str_ymdhms(" obs removed ") + satells);
        }
    ...
    if( _log && _log->verb() >= 3 ) _log->comment(3,"gallobs", site + tt.str_ymdhms(" skipped ") + " " + sat);
...
int t_gallobs::_find_epo(const string& site, const t_gtime& epo, t_gtime& tt)
{
  ...
  if( it1 == _mapobj[site].end() && it0 == _mapobj[site].end() ){
    if( _log ) _log->comment(2,"gallobs", "no observations for site " + site + epo.str_ymdhms(" epoch: "));
    return -1;
  }
