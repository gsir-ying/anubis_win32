...
{ 
  ...
  // -------- "GLONASS COD/PHS/BIS --------
    ...
    vector<GOBS> v_obs = { C1C, C1P, C2C, C2P };
    for(size_t ii=0; ii < v_obs.size(); ++ii){                  // single line (4 fields or less)

      string  obs = trim(_line.substr(1 + 13*(ii%(4)), 3));
      if( obs.empty() ){ _globia.push_back(make_pair(v_obs.at(ii),0.0)); continue; }
      string  val = trim(_line.substr(5 + 13*(ii%(4)), 8));
      GOBS   gobs = str2gobs( obs );
      double bias = str2dbl( val );

#ifdef DEBUG
      cout << "gobs: " << gobs2str(gobs) << " bias:" << bias << " nobs:" << ii << endl;
#endif
    ...
    if( _log && _log->verb() >= 2 ){
      ...
      for( auto itOBS = _globia.begin(); itOBS!= _globia.end(); ++itOBS ){
...
//  if( _log ) _log->comment(3,"rinexo3",_epoch.str_ymdhms("RINEX 3.x special events not handled after "));
...
int t_rinexo3::_read_obstypes( const string& sat, const string& sys )
{
  ...
  if( ! filter_sat ){ _xsys++; if( _log ) _log->comment( 2, "rinexo3", "skip "+tmpsat); }
...
int t_rinexo3::_fix_band(string sys, string& go)
{
  ...
  {
    if( _version.compare("3.02") == 0 ){ // change B1 -> B2 !!!
      if( go[1] == '1' ){  
        go[1] = '2';
