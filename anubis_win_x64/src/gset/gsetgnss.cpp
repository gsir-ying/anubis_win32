...
{   
  ...
  for( itGNS = gnss_sats.begin(); itGNS != gnss_sats.end(); ++itGNS ){
    ...
    set<string>             sats = t_gsetbase::_setval( lc(_gsys(gsys)), "sat" );
    if( sats.size() == 0 ){ sats = t_gsetbase::_setval( uc(_gsys(gsys)), "sat" ); }
...
set<string> t_gsetgnss::sat(GSYS gsys, bool def) // default=true enforce to fill 
{
  ...
  set<string>            tmp = t_gsetbase::_setval( lc(_gsys(gsys)), "sat" );
  if( tmp.size() == 0 ){ tmp = t_gsetbase::_setval( uc(_gsys(gsys)), "sat" ); }
...
{
  _gmutex.lock();  
  set<string>            tmp = t_gsetbase::_setval( lc(_gsys(gsys)), "nav" );
  if( tmp.size() == 0 ){ tmp = t_gsetbase::_setval( uc(_gsys(gsys)), "nav" ); }
...
{   
  ...
  vector<string> type = t_gsetbase::_vecval( lc(_gsys(gsys)), "type" );
  vector<string> band = t_gsetbase::_vecval( lc(_gsys(gsys)), "band" );
  vector<string> attr = t_gsetbase::_vecval( lc(_gsys(gsys)), "attr" );

  if( type.size() == 0 ){ type = t_gsetbase::_vecval( uc(_gsys(gsys)), "type" ); }
  if( band.size() == 0 ){ band = t_gsetbase::_vecval( uc(_gsys(gsys)), "band" ); }
  if( attr.size() == 0 ){ attr = t_gsetbase::_vecval( uc(_gsys(gsys)), "attr" ); }
...
{   
  ...
  vector<string>           vgobs = t_gsetbase::_vecval( lc(_gsys(gsys)), "gobs" );
  if( vgobs.size() == 0 ){ vgobs = t_gsetbase::_vecval( uc(_gsys(gsys)), "gobs" ); }
...
{ 
  vector<string>           v_str = t_gsetbase::_vecval( lc(_gsys(gsys)), "type" );
  if( v_str.size() == 0 ){ v_str = t_gsetbase::_vecval( uc(_gsys(gsys)), "type" ); }
    
  vector<GOBSTYPE> v_tmp;
  for( vector<string>::const_iterator it = v_str.begin(); it != v_str.end(); ++it ){
    string str = *it; 
    transform(str.begin(), str.end(), str.begin(), ::toupper);
    GOBSTYPE gobstype = char2gobstype( str[0] );
...
{   
  vector<string>           v_str = t_gsetbase::_vecval( lc(_gsys(gsys)), "band" );
  if( v_str.size() == 0 ){ v_str = t_gsetbase::_vecval( uc(_gsys(gsys)), "band" ); }
  vector<GOBSBAND> v_tmp;
  for( vector<string>::const_iterator it = v_str.begin(); it != v_str.end(); ++it ){
    string str = *it;
    transform(str.begin(), str.end(), str.begin(), ::toupper);
    GOBSBAND gobsband = char2gobsband( str[0] );
...
{
  vector<string>           v_str = t_gsetbase::_vecval( lc(_gsys(gsys)), "attr" );
  if( v_str.size() == 0 ){ v_str = t_gsetbase::_vecval( uc(_gsys(gsys)), "attr" ); }
  vector<GOBSATTR> v_tmp;
  for( vector<string>::const_iterator it = v_str.begin(); it != v_str.end(); ++it ){
    string str = *it; 
    transform(str.begin(), str.end(), str.begin(), ::toupper);
    GOBSATTR gobsattr = char2gobsattr( str[0] );
...
{
  double sig = 0.0; sig = t_gsetbase::_dblatt( lc(_gsys(gsys)), "sigma_C" );
  if( sig == 0.0 ){ sig = t_gsetbase::_dblatt( uc(_gsys(gsys)), "sigma_C" ); }
...
{
  double sig = 0.0; sig = t_gsetbase::_dblatt( lc(_gsys(gsys)), "sigma_L" );
  if( sig == 0.0 ){ sig = t_gsetbase::_dblatt( uc(_gsys(gsys)), "sigma_L" ); }
...
{
  double res = 0.0; res = t_gsetbase::_dblatt( lc(_gsys(gsys)), "maxres_C" );
  if( res == 0.0 ){ res = t_gsetbase::_dblatt( uc(_gsys(gsys)), "maxres_C" ); }
...
{
  double res = 0.0; res = t_gsetbase::_dblatt( lc(_gsys(gsys)), "maxres_L" );
  if( res == 0.0 ){ res = t_gsetbase::_dblatt( uc(_gsys(gsys)), "maxres_L" ); }
