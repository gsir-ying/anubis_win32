...
cerr << "REC: " << fixed << setprecision(3)
  ...
  t_gtriple std(999, 999, 999);
  tmp->crd( xyz, std,                      beg, end); // FULL TIME
  
  tmp->ecc( ecc, t_gobj::ECC_NEU,          beg, end); // FULL TIME
  ...
  for( xml_node set = site.child("set"); set; set = set.next_sibling("set") ){ // RETURNS REDUNDANT --> problem? !!!!
    ...
    if( !dx.empty() && !dy.empty() && !dz.empty()) { 
      ...
      xyz[1] = str2dbl(dy);
      xyz[2] = str2dbl(dz);
      tmp->ecc(xyz,t_gobj::ECC_XYZ,beg,end);
    }

    string dn = set.attribute("DN").value();
    string de = set.attribute("DE").value();
    string du = set.attribute("DU").value();
    if( !dn.empty() && !de.empty() && !du.empty()) {
      xyz[0] = str2dbl(dn);
      xyz[1] = str2dbl(de);
      xyz[2] = str2dbl(du);
      tmp->ecc(xyz,t_gobj::ECC_NEU,beg,end);       
    }     
