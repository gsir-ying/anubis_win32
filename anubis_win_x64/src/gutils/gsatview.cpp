...
// calculate satellite visibility
// ----------
int sat_data( t_spt_qcdata     qc_data,
              t_gtriple        xyz_rec,
              t_gallnav*       all_nav,
              const t_gtime&   beg,       // should be setup reasonably !
              const t_gtime&   end,       // should be setup reasonably !
            ...
            )
{
  ...
  if( beg > end || beg == FIRST_TIME || end == LAST_TIME ){ return -1; }
  if( !all_nav || !qc_data ){ return -1; }
...
//  double tdif = 0.0;
  ...
  for( auto itSYS = gnss_sats.begin(); itSYS != gnss_sats.end(); ++itSYS )
  { 
    GSYS gs = itSYS->first;

    for( auto itSAT = gnss_sats[gs].begin(); itSAT != gnss_sats[gs].end(); ++itSAT )
    { 
      string prn = *itSAT;
      ...
      while( tt < end_lim + 1 )
      {
        ...
        cout << tt.str_ymdhms() << " " << prn << " ele: " << ele << endl;
        if( ele > 0 ){ qc_data->qc_tim.dat[gs][tt][prn].ele = ele;  }
      }       
    }
  }
